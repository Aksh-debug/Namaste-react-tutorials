...
const RestaurantMenu = () => {
  ...
  return (
    <div className="w-[800px] ml-72 mt-8">
      ...
      <h2 className=" my-8  mx-4 text-lg text-[#3e4152] font-bold">Top Picks</h2>
      {categories.map((category,index)=>(
        <RestaurantCategory key={category?.card?.card?.title} data={category} showItems={index===showIndex ? true : false} index={index} setShowIndex={(idx)=>setShowIndex(idx)}/>
      ))}
    </div>
  );
};
